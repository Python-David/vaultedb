...
from vaultdb.encrypted_storage import ExportFormat
...
def test_export_key_default_returns_dict(temp_vault_path):
    vault = VaultDB.open(temp_vault_path, "hunter2")
...
def test_export_key_json_writes_file_and_returns_path(temp_vault_path):
    ...
    temp_file.close()
    os.remove(filepath)  # allow VaultDB to write
